...
iters = args.iters
ref_str = args.ref_strategy

files = glob.glob(folder_dir + "/*.txt")

case = args.coarsen_refine
needed_files = [
    f"output_{ref_str}_L2_{case}.txt",
    f"output_{ref_str}_euclidean_{case}.txt",
]
labels = [
    f"Ref strategy {ref_str} in $L^2$",
    f"Ref strategy {ref_str} in $\ell^2$",
]
...
iter_cut = iters
for i, my_color, label in zip(range(2), color_to_plot, labels):
